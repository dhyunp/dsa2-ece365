  ...
  private:
    class node{
        public:
            ...
            int key; //node key
            void *pointData; //pointer to the actual data
    };

    int heapCapacity;              //capacity of heap
    int heapSize;                  //current size of heap
    std::vector<node> heapData;    //acutal data of heap
    hashTable *allocate;         //allocates ids to node pointers in a hash table
